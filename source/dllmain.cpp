...
const std::vector<BYTE> commonHexEdit = { 0x20, 0x03, 0x00, 0x00, 0xC7, 0x44, 0x24, 0x18, 0x80, 0x02, 0x00, 0x00, 0x8B, 0x44, 0xB4, 0x0C, 0xA3, 0x68, 0x23, 0x5E,
0x00, 0xA3, 0x6C, 0xE9, 0x64, 0x00, 0xA0, 0x5D, 0x4E, 0x5F, 0x00, 0xF6, 0xD8, 0xC7, 0x44, 0x24, 0x24, 0x00, 0x03, 0x00, 0x00, 0xC7, 0x44, 0x24, 0x28, 0x58, 0x02 };
...
HexEdit CreateHexEditForResolution(int width, int height, size_t offset) {
    HexEdit edit;
    edit.offset = offset;

    // Convert width and height to two-byte little-endian values
    ...
    // Construct the modified vector
    edit.modified = 
    { widthLow, widthHigh, 0x00, 0x00, 0xC7, 0x44, 0x24, 0x18, 0x80, 0x02, 0x00, 0x00, 0x8B, 0x44, 0xB4, 0x0C, 0xA3, 0x68, 0x23, 0x5E,
0x00, 0xA3, 0x6C, 0xE9, 0x64, 0x00, 0xA0, 0x5D, 0x4E, 0x5F, 0x00, 0xF6, 0xD8, 0xC7, 0x44, 0x24, 0x24, 0x00, 0x03, 0x00, 0x00, 0xC7, 0x44, 0x24, 0x28, heightLow, heightHigh };
    ...
    for (DWORD i = 0; i < moduleSize - commonHexEdit.size(); ++i) {
        if (memcmp(lpAddress + i, commonHexEdit.data(), commonHexEdit.size()) == 0) {
            ...
            if (!result || numberOfBytesWritten != edit.modified.size()) {
                ...
                // Restore original protection in case of failure
                VirtualProtect(lpAddressToWrite, edit.modified.size(), oldProtect, &oldProtect);
                return;
            }

            // Restore original protection
            if (!VirtualProtect(lpAddressToWrite, edit.modified.size(), oldProtect, &oldProtect)) {
...
void PerformHexEdits() {
    ...
    strcpy(strrchr(path, '\\'), "\\d3d9.ini");
        ...
        // Read width and height from the INI file
        int width = GetPrivateProfileInt("RESOLUTION", "width", 0, path);
    int height = GetPrivateProfileInt("RESOLUTION", "height", 0, path);

    if (width == 0 || height == 0) {
        DX_ERROR("Failed to read width or height from INI file.");
        return;
    }

    // Create hex edits for resolution
    HexEdit resolutionEdit = CreateHexEditForResolution(width, height, 0);

    // Perform hex edits
    PerformHexEdit(lpAddress, moduleSize, resolutionEdit);
}
...
void PerformHexEdit2(LPBYTE lpAddress, DWORD moduleSize) {
    ...
    // Define the edits
    std::vector<HexEdit2> edits2 = {
        { { 0x7C, 0x46, 0x83, 0xF9, 0x06, 0x7D, 0x41 }, { 0x7C, 0x00, 0x83, 0xF9, 0x06, 0x7D, 0x00 }, 0 },
    };

    // Iterate through the edits
    for (const auto& edit2 : edits2) {
        // Search for the pattern in memory
        for (DWORD i = 0; i < moduleSize - edit2.pattern.size(); ++i) {
            if (memcmp(lpAddress + i, edit2.pattern.data(), edit2.pattern.size()) == 0) {
                // Pattern found in memory
                DX_PRINT("Pattern found in memory.")

                    // Modify memory
                    LPVOID lpAddressToWrite = lpAddress + i + edit2.offset;
                SIZE_T numberOfBytesWritten;
                DWORD oldProtect;
                if (!VirtualProtectEx(GetCurrentProcess(), lpAddressToWrite, edit2.newValue.size(), PAGE_EXECUTE_READWRITE, &oldProtect)) {
                    DX_ERROR("Failed to change memory protection.")
                        return;
                }

                BOOL result = WriteProcessMemory(GetCurrentProcess(), lpAddressToWrite, edit2.newValue.data(), edit2.newValue.size(), &numberOfBytesWritten);
                if (!result || numberOfBytesWritten != edit2.newValue.size()) {
                ...
                // Restore original protection
                DWORD dummy;
                if (!VirtualProtectEx(GetCurrentProcess(), lpAddressToWrite, edit2.newValue.size(), oldProtect, &dummy)) {
                    DX_ERROR("Failed to restore memory protection.")
                        return;
                }
...
void PerformHexEdits2() {
    ...
    PerformHexEdit2(lpAddress, moduleSize);
}
...
void PerformHexEdit3(LPBYTE lpAddress, DWORD moduleSize) {
    ...
    std::vector<HexEdit3> edits3 = {
        { { 0x7C, 0x00, 0x83, 0xF9, 0x06, 0x7D, 0x00 }, { 0x7C, 0x46, 0x83, 0xF9, 0x06, 0x7D, 0x41 }, 0 },
    };
    ...
    for (const auto& edit3 : edits3) {
        ...
        for (DWORD i = 0; i < moduleSize - edit3.pattern.size(); ++i) {
            if (memcmp(lpAddress + i, edit3.pattern.data(), edit3.pattern.size()) == 0) {
                ...
                SIZE_T numberOfBytesWritten;
                DWORD oldProtect;
                if (!VirtualProtectEx(GetCurrentProcess(), lpAddressToWrite, edit3.newValue.size(), PAGE_EXECUTE_READWRITE, &oldProtect)) {
                    DX_ERROR("Failed to change memory protection.")
                        return;
                }
                ...
                // Restore original protection
                DWORD dummy;
                if (!VirtualProtectEx(GetCurrentProcess(), lpAddressToWrite, edit3.newValue.size(), oldProtect, &dummy)) {
                    DX_ERROR("Failed to restore memory protection.")
                        return;
                }
...
const std::vector<BYTE> commonHexEdit4 = { 0x10, 0x50, 0xFF, 0x92, 0xE4, 0x00, 0x00, 0x00, 0x68, 0x00, 0x00, 0x80, 0xBF };
...
HexEdit4 CreateHexEditFromFOV(int FOVIndex) {
    HexEdit4 edit;

    switch (FOVIndex) {
    case 1:
        edit.modified = { 0x10, 0x50, 0xFF, 0x92, 0xE4, 0x00, 0x00, 0x00, 0x68, 0x00, 0x00, 0x90, 0x3F };  // 16:10
        edit.offset = 0;
        break;
    case 2:
        edit.modified = { 0x10, 0x50, 0xFF, 0x92, 0xE4, 0x00, 0x00, 0x00, 0x68, 0x00, 0x00, 0x80, 0x3F };  // 16:9
        edit.offset = 0;
        break;
    case 3:
        edit.modified = { 0x10, 0x50, 0xFF, 0x92, 0xE4, 0x00, 0x00, 0x00, 0x68, 0x00, 0x00, 0x40, 0x3F };  // 21:9
        edit.offset = 0;
        break;
    case 4:
        edit.modified = { 0x10, 0x50, 0xFF, 0x92, 0xE4, 0x00, 0x00, 0x00, 0x68, 0x00, 0x00, 0xB0, 0x3E };  // Eyefinity / Surround
        edit.offset = 0;
        break;
    default:
        DX_PRINT("Invalid FOV index.")
            break;
    }
...
void PerformHexEdit4(LPBYTE lpAddress, DWORD moduleSize, const HexEdit4& edit) {
    for (DWORD i = 0; i < moduleSize - commonHexEdit4.size(); ++i) {
...
void PerformHexEdits4() {
    ...
    HexEdit4 edit = CreateHexEditFromFOV(FOVIndex);
